...
def index():
	...
	if request.method == 'POST':
		...
		if choice is None:
			...
			# User selected a new sheet
			# Update the current sheet name in the hidden input field
			current_sheet = choice
		...
		print("INDEX FORMAT")
		print(df.index.format)
		df.index = pd.to_datetime(df.index.map(lambda x: '-'.join(map(str, x)) + '-01'), format='%Y-%B-%d')
